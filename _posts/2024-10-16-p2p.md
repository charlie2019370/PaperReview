...
First, let $$DM(z_t,P,t,s)$$ be a single diffusion step, outputting noisy image $$z_{t-1}$$ and attention map $$M_t$$. Then, let $$DM(z_t,P,t,s)\{M \larr \hat{M}\}$$ the step of replacing original attention map M with an additional attention map $$\hat{M}$$. 
