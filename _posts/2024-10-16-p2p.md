...
![algo]({{ '/assets/img/p2p/algo.png' | relative_url }}){: width=150px}

We can use the original image I as an additional input without changing the algorithm.

**Word Swap**

This case is swapping an word of the prompt into another word. The attention maps of the original image are injected into generation with edited prompt, but to preserve the structure of the original image, P2P proposes a softer attention constrain defined as follows: 
![swap]({{ '/assets/img/p2p/swap.png' | relative_url }})
Here $$\tau$$ is determines until which timestep the injection of the original attention maps is applied.
